...
const {
	port = 80, language = 'en',
	disableMarks = false, disableMotivationalQuotes = false, disableTests = false, disableResources = false
...
app.get('/users/:id', (req, res) => {
	...
	else res.respond('', 'users/noone.webp', 'image/webp', 200)
})
...
app.post('/students/tests/getQuestions', (req, res) => {
	if (req.userInfo.role != "student" ||
		...
		importJSON(`test-progress/${req.userInfo.userID}/${req.body.ID}.json`).finished == true) return res.sendError(403)
	let classes = importJSON('classes.json')
	let subjects = importJSON('subjects.json')
	let users = importJSON('users.json')
	let test = {}
...
app.post('/parents/marks/get', (req, res) => {
	if (req.userInfo.role != "parent") return res.sendError(403)
	if (req.userInfo.children.findIndex(e => e.userID == req.cookies.selectedChild) == -1) return res.sendError(500)
	let exists = false
	let marks = importJSON('marks.json')
	let resContent = importJSON('periods.json')
	let subjects = importJSON('subjects.json')
...
app.post('/parents/marks/graph', (req, res) => {
	if (req.userInfo.role != "parent" || req.userInfo.children.findIndex(e => e.userID == req.body.studentID) == -1) return res.sendError(403)
	let exists = false
	let marks = importJSON('marks.json')
	let resContent = []
	let subjects = importJSON('subjects.json')
...
app.post('/teachers/marks/delete', (req, res) => {
	...
	if (markIndex == -1) return res.sendError(400)
	let mark = marks[markIndex]
	if (mark.ownerID != req.userInfo.userID) res.sendError(403)
...
app.post('/teachers/listMyStudents', (req, res) => {
	...
	res.respond(JSON.stringify({ students: theirStudents.sort(sortByPrettyName), subjectsByClass }), '', 'application/json', 200)
})
...
app.post('/teachers/tests/getAnswers', (req, res) => {
	if (req.userInfo.role != "teacher") return res.sendError(403)
	if (!req.body.studentID || !req.body.testID) return res.sendError(403)
...
app.post('/teachers/tests/setOpenAnswerAs', (req, res) => {
	if (req.userInfo.role != "teacher") return res.sendError(403)
	if (req.body.studentID == undefined || req.body.testID == undefined || req.body.questionI == undefined || req.body.correct == undefined) return res.sendError(403)
...
app.post('/teachers/tests/listSubmissions', (req, res) => {
	if (req.userInfo.role != "teacher") return res.sendError(403)
	if (req.body.testID == undefined) return res.sendError(400)
	let submissions = []
	let users = importJSON('users.json')
	readdirSync('test-progress').forEach(student => {
...
app.post('/misc/notifications/get', (req, res) => {
	...
	let locales = eval(`importJSON('localization.json').${language}`)
...
app.post('/manager/users/delete', (req, res) => {
	if (!req.userInfo.isAdmin) res.sendError(403)
...
app.post('/manager/users/deleteAll', (req, res) => {
	if (!req.userInfo.isAdmin) res.sendError(403)
	saveJSON('users.json', [])
	res.respond(JSON.stringify({ message: 'ok' }), '', 'application/json', 200)
})
...
app.post('/manager/classes/new', (req, res) => {
	if (!req.userInfo.isAdmin) res.sendError(403)

	let classes = importJSON('classes.json')
	let newClassID = v4()
	let newClass = {
        prettyName: "New Class",
        classID: newClassID,
        students: []
	}
	...
	saveJSON('classes.json', classes)
	res.respond(JSON.stringify(newClass), '', 'application/json', 200)
})
app.post('/manager/classes/delete', (req, res) => {
	if (!req.userInfo.isAdmin) res.sendError(403)
...
app.listen(port, () => console.log(`[Kuzi] Listening on port ${port}`))