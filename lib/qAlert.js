...
qAreYouSure = (options = { message: '[{(areYouSure)}]' }) => qAlert({ mode: 'question', buttons: { ok: { text: '[{(yes)}]' }, cancel: { text: '[{(no)}]' } }, ...options })
qError = (options = { message: '[{(error.unknown)}]' }) => qAlert({ mode: 'error', buttons: { cancel: { invisible: true } }, ...options }).then(() => { if (options.goBack === true) history.back() })
qSuccess = options => qAlert({ mode: 'success', buttons: { cancel: { invisible: true } }, ...options })