    ...
    qAlert_alert.outerHTML = `
        <div id="qAlert" style="display: none;">
            <div class="qAlert-inner">
                ...
                <button class="btn ok">qAlert.default.ok</button>
                <button class="btn cancel">qAlert.default.cancel</button>
            </div>
        </div>
    `
    $('#qAlert .text-input input').addEventListener('keydown', e => { if (e.key == "Enter") $("#qAlert .ok").click() } )
})
